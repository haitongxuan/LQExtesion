...
namespace T4
{
    public class Config
    {
        ...
        //需要生成的表名，多张表用,分割
        public static readonly string Tables = "";//"users,rows,titles"
    }
    ...
    public abstract class BaseDbHelper
    {
        ...
        public List<DbColumn> DtColToList(DataTable dt, ISqlServerDbTypeMap sqlDbTypeMap)
        {
            ...
            foreach (var row in dr)
            {
                ...
                if(row["IsForeignKey"]!=null && row["IsForeignKey"].ToString() != "")
                {
                    dbCoModel.IsForeignKey = StrToBool(row["IsForeignKey"].ToString());
                }
                if (row["PkeyTableName"] != null)
                {
                    dbCoModel.PkeyTableName = row["PkeyTableName"].ToString();
                }
    ...
    public sealed class DbColumn
    {
        ...
        /// 外键对应主键表
        /// </summary>
        public string PkeyTableName { get; set; }
        /// <summary>
        /// 字段名称
        /// </summary>
