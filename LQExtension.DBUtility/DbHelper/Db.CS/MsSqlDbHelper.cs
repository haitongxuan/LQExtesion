        ...
        public override List<DbColumn> GetDbColumns(string tableName, string schema = "dbo")
        {
            ...
            string sql = string.Format(@"
                                    ...
                                    (
		                                ...
		                                FkCTE as (
			                            SELECT 
	                            b.fkey,b.rkey,b.fkeyid,
	                            FkeyTableName=object_name(b.fkeyid),
	                            PkeyTableName=object_name(b.rkeyid)
                            FROM sysobjects a   
                                join sysforeignkeys b on a.id=b.constid   
                                join sysobjects c on a.parent_obj=c.id  
	                            where b.fkeyid=OBJECT_ID(@tablename)
		                            )
                                    select
		                            colm.column_id ColumnID,
                                    CAST(CASE WHEN indexCTE.column_id IS NULL THEN 0 ELSE 1 END AS BIT) IsPrimaryKey,
		                            CAST(case when FkCTE.fkey is null then 0 else 1 end as bit) IsForeignKey,
		                            FkCTE.PkeyTableName,
		                            ...
		                            left join FkCTE on colm.column_id=FkCTE.fkey and colm.object_id=FkCTE.fkeyid		                                     
